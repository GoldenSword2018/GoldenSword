...
void CoreObject::Render()
{
	...
	if (this->pArmor_Index.size() > 0)
	{
		...
		*/
		...
		D3DXMATRIXA16 mtxTransform;
		D3DXMATRIXA16 mtxRotationY;
		D3DXMATRIXA16 mtxRotationAxis;
		D3DXVECTOR3 vecFaceGroud;
		D3DXVECTOR3 vecRight;

		D3DXMatrixTranslation(&mtxTranslation, 0.0f, -0.5f, 0.0f);	// 原点に平行移動
		D3DXMatrixRotationY(&mtxRotation, D3DX_PI);					// Y軸周りに半回転して正面に向ける
		D3DXMatrixScaling(&mtxScaling, 0.4f, 0.4f, 0.4f);			// サイズ調整

		mtxTransform = mtxTranslation * mtxRotation * mtxScaling;	// 基準変換行列の設定
		...
		//合成
		this->transform.MtxWorld = mtxWorld = mtxTransform * mtxRotation * mtxTranslation;
